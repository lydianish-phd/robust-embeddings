...
if __name__ == "__main__":
    ...
    for seed in range(100,110):    
        corpus = f"{CORPUS}/{seed}"
        for corpus_part in corpus_parts:
            results = []
            files = [ file for file in os.scandir(os.path.join(args.input_dir, corpus, corpus_part)) if file.name.endswith(f"{corpus_part}.tok") ]
            for file in files:
                print(file.name)
            ...
            results_df.to_csv(os.path.join(args.input_dir, corpus, corpus_part, _file(metric)))
    
    print("Aggregating results...")
    for corpus_part in corpus_parts:
        output_file = os.path.join(args.input_dir, CORPUS, corpus_part + '-' + _file(metric))
        all_data = pd.DataFrame(columns=["file", "types", "tokens", "ttr"])
        for seed in range(100,110):
            corpus = f"{CORPUS}/{seed}"
            input_file = os.path.join(args.input_dir, corpus, corpus_part, _file(metric))
        ...
        for metric in all_data.columns[1:]:
            average_output_file = os.path.join(args.input_dir, CORPUS, corpus_part + '-avg-' + _file(metric))
