...
if __name__ == "__main__":
    ...
    for seed in range(100,110):   
        ...
        for corpus_part in corpus_parts:
            results = []
            files = [ file for file in os.scandir(os.path.join(args.input_dir, str(seed), corpus_part)) if file.name.endswith(f"{corpus_part}.tok") ]
            for file in files:
                print(f"\t{file.name}")
            ...
            results_df.to_csv(os.path.join(args.input_dir, str(seed), corpus_part, _file(metric)))
    ...
    for corpus_part in corpus_parts:
        ...
        for metric in all_data.columns[1:]:
            ...
            target_mean = all_data[all_data["file"]==f"{gold_file}.{corpus_part}"][_name(metric)].mean()
